      ...
      <VCardText>
        ...
        <VRow v-if="items.length > 0" class="mb1-2">
          <VCol v-for="kasir in kasirs" :key="kasir.username" cols="4"
            >{{ kasir.username }} - {{ formatCurrency(kasir.amount) }}</VCol
          >
          <VCol class="text-right font-bold" cols="4"
            >Total: {{ formatCurrency(totalAmount) }}</VCol
          >
        </VRow>
    ...
    .then((response) => {
      if (response.status === 200) {
        ...
        if (items.value.length === 0) {
          appStore.showSnackbar('Tidak ada transaksi untuk tanggal tersebut.', 'info')
        } else {
          const temp = {}
          items.value.forEach((item) => {
            if (item.user && item.user.username) {
              if (!temp[item.user.username]) {
                temp[item.user.username] = { username: item.user.username, amount: 0 }
              }
